...
for file_name in os.listdir(path):
    ...
    for nb_run in range(1):
        ...
        for i in range(len(list_node)):
            ...
            if list_node[i].avg_energy > 1.5 or list_node[i].energy<5000:
                list_node[i].avg_energy = 1 + 0.3 * random.random()
                list_node[i].energy = 6000 + random.random() * 500
            if list_node[i].energy_max == list_node[i].energy:
                count_disconnected += 1
                continue
            if (list_node[i].energy - para.min_energy) / list_node[i].avg_energy < 20000:
                count_weak += 1

        for i in range(len(list_node)):
            if list_node[i].energy_max == list_node[i].energy:
                if (count_weak < len(list_node)/5):
                    list_node[i].energy = 7500 + random.random() * 1500
                    list_node[i].avg_energy = 0.25 + 0.3 * random.random()
                else :
                    list_node[i].energy = 8000 + random.random() * 1500
                    list_node[i].avg_energy = 0.05 + 0.06 * random.random()
            else:
                continue
            if (list_node[i].energy - para.min_energy) / list_node[i].avg_energy < 20000:
                count_weak += 1
        for i in range(len(list_node)):
            out.write(str(list_node[i].location[0]) + '\t' + str(list_node[i].location[1]) + '\t' + str(
                "{:.4f}".format(list_node[i].energy)) + '\t' + str("{:.4f}".format(list_node[i].avg_energy)) + '\n')
